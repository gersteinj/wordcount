...
def clean(raw_string):
    ...
    for char in raw_string:
        ...
        if char in string.ascii_letters:
            ...
            logger.debug('space')
            cleaned_text += char
        # If character is whitespace other than space, convert to space
...
def wordcount(raw_string):
    """Takes a multi-word string and returns a dictionary of counted words"""
    return count_words(split_words(clean(raw_string)))


def unique_words(source, comparison):
    """Takes two dictionaries and returns words found in the first but not the second"""
    uniques = []
    for key in source.keys():
        if key not in comparison.keys():
            uniques.append(key)
    return uniques
